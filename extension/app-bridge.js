...
function announceVersion() {
  window.postMessage({ type: READY_TYPE, version: chrome.runtime.getManifest().version }, window.location.origin);
}
...
window.addEventListener("message", (event) => {
  if (event.source === window && event.origin === window.location.origin && event.data?.type === PING_TYPE) announceVersion();
});
