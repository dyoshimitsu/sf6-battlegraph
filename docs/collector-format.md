...
`npm run build` produces `dist/extension` and `dist/sf6-battlegraph-connector-v<version>.zip`. The ZIP and its single top-level directory use the same versioned name. Neither output contains credentials.
