...
Bucklerの`window.opener`が別origin間で維持されることに依存しない。Battlegraph側content script、extension service worker、Buckler側content scriptをruntime messageで接続し、service workerが収集ごとに起点タブとBucklerタブを対応付ける。取得bundleは対応する起点タブだけへ返す。
...
Chrome拡張はブラウザへ一度導入すればセッションをまたいで維持されるため、接続カードを通常画面へ常設しない。取得・同期操作は管理者ヘッダーへ集約する。配布ZIPのダウンロード名にはversionと日時を含め、ZIP内はバージョン付き単一フォルダにする。導入・更新リンクを押した後は、展開、旧拡張の削除、新フォルダの読み込みという3手順を画面内に表示する。
...
LP・MR推移はランクマッチだけを対象とし、選択キャラクターについてフィルタ後の直近100試合を表示する。初期対象は最新のランクマッチで使用したキャラクターとし、手動選択後はその選択を維持する。カジュアルやバトルハブのレスポンスにLP・MR現在値が含まれていても推移には使用しない。日別記録は最新対戦日を終点とする直近14暦日を表示し、対戦しなかった日も0試合として含める。

正規化済み試合に対するクライアント集計も実装した。

- 期間は `Asia/Tokyo` の暦日で両端を含めて絞り込む
- モードと使用キャラクターを日付範囲と組み合わせて絞り込む
- 総合履歴の各試合は`replay_battle_type`と名称から実際のモードへ正規化する。過去に`all`で保存したquery matchも読み込み時に同じ規則で補正する
- 全体、使用キャラクター別、相手キャラクター別の勝敗と勝率を計算する
- 追跡対象が1P側・2P側のどちらだったかを試合一覧へ表示し、各sideの勝率を別々に集計する。query chunk schema v2以降で`subjectSide`を保持し、旧chunkは誤推定せず不明として扱う
- 旧query chunkに`subjectSide`がない場合、通常閲覧では追加readを行わない。次の管理者同期時だけ不足する完全matchを最大30 IDずつ問い合わせ、sideを補完した新generationへ移行する
...
同期完了時は、今回初めて保存した試合、既存と重複して再取得した試合、今回のBuckler履歴には含まれないが保存を維持した過去試合、保存後の合計件数をトーストで表示する。自動同期の途中に同期前プレビュー画面は挟まない。
...
- 未観測の `replay_battle_type` と全 `replay_battle_sub_type` の対応
...
初回同期は既知のreplayがないため総合履歴の全ページを取得する。2回目以降は保存済み履歴の新しい20 replay IDをURL fragmentでcollectorへ渡し、いずれかが現れたページを丸ごと保存して取得を終了する。境界IDが削除・変動して見つからない場合は従来どおり最終ページまで取得する。bundleには正常な早期終了理由と実際に発見したIDを記録し、parserがそのIDの存在を再検証する。
