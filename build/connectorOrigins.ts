...
export function buildConnectorMatchPatterns(value?: string): string[] {
  const origins = value ? value.split(",").map(origin => origin.trim()).filter(Boolean) : DEFAULT_CONNECTOR_ORIGINS;
  if (origins.length === 0) throw new Error("VITE_CONNECTOR_ORIGINS must contain at least one origin");
