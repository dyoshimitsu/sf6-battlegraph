...
function match(overrides: Partial<NormalizedMatch>): NormalizedMatch {
  ...
  return {
    ...
    raw: {
      ...
      uploaded_at: 1,
      player1_info: player,
      player2_info: { player: { short_id: 2 } },
    },
    ...overrides,
  };
}
...
describe("ratingMatches", () => {
  ...
  it("selects the character used in the latest ranked match", () => {
    const older = match({
      playedAtEpoch: 100,
      battleType: 1,
      subject: { player: { short_id: 1 }, character_id: 21 },
    });
    const latest = match({
      playedAtEpoch: 300,
      battleType: 1,
      subject: { player: { short_id: 1 }, character_id: 2 },
    });
    const newerCasual = match({
      playedAtEpoch: 400,
      battleType: 2,
      battleTypeName: "CASUAL MATCH",
      subject: { player: { short_id: 1 }, character_id: 3 },
    });
