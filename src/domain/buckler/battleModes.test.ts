...
describe("inferBattleMode", () => {
  it.each([
    ...
    expect(inferBattleMode({ replay_battle_type: type, replay_battle_type_name: name }, "all")).toBe(expected);
  });

  it("prefers a recognized name and preserves unknown values", () => {
    expect(inferBattleMode({ replay_battle_type: 99, replay_battle_type_name: "RANKED MATCH" }, "all")).toBe("ranked");
    expect(inferBattleMode({ replay_battle_type: 99, replay_battle_type_name: "NEW MODE" }, "all")).toBe("unknown");
