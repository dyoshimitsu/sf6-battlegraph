...
describe("Firestore archive restore", () => {
  it("validates the player and always places the manifest last", () => {
    ...
    expect(plan.writesBeforeManifest.map((write) => write.path)).not.toContain(
      "players/100/manifests/matches",
    );
  ...
  it("uses the current deployment visibility instead of a value stored in the backup", () => {
    ...
    expect(
      plan.writesBeforeManifest.filter((write) => write.path === "settings/deployment"),
    ).toEqual([{ path: "settings/deployment", data: { visibility: "private" } }]);
  });

  it("batches data and activates the manifest only after every data batch", async () => {
    const value = archive();
    value.documents.splice(
      1,
      0,
      ...Array.from({ length: MAX_RESTORE_WRITES_PER_BATCH }, (_, index) => ({
        path: `players/100/matches/X${index}`,
        data: { index },
      })),
    );
    value.documentCount = value.documents.length;
