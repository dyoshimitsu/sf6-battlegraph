...
export function buildRestorePlan(
  ...
  if (archive.userCode !== expectedUserCode)
    throw new Error(`Archive user code ${archive.userCode} does not match ${expectedUserCode}`);
  ...
  const writesBeforeManifest = archive.documents.filter(
    (document) => document.path !== manifestPath && document.path !== "settings/deployment",
  );
  ...
  return {
    ...
    manifest,
    writeCount: writesBeforeManifest.length + 1,
  };
}

export async function executeRestorePlan(
  port: RestoreWritePort,
  plan: RestorePlan,
  ...
  for (
    let offset = 0;
    offset < plan.writesBeforeManifest.length;
    offset += MAX_RESTORE_WRITES_PER_BATCH
  ) {
