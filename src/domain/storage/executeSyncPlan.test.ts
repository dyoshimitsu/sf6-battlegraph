...
function plan(dataWriteCount: number): SyncPlan {
  const writesBeforeManifest = Array.from(
    { length: dataWriteCount },
    (_, index): PlannedWrite => ({ path: `data/${index}`, data: { index } }),
  );
  return {
    syncId: "sync",
    ...
    manifest: { path: "manifest/active", data: {} },
    deletesAfterManifest: [],
    completionWrites: [
      { path: "snapshots/sync", data: { status: "complete" } },
      { path: "syncs/sync", data: { status: "complete" } },
    ],
    writeCount: dataWriteCount + 3,
    storedMatches: [],
  };
}
...
describe("executeSyncPlan", () => {
  ...
  it("reports progress through final activation", async () => {
    ...
    await executeSyncPlan(
      { commit: async () => undefined, remove: async () => undefined },
      plan(2),
      (item) => progress.push(`${item.phase}:${item.completed}/${item.total}`),
    );
  ...
  it("activates the new manifest before deleting obsolete chunks and then clears the pending list", async () => {
    ...
    await executeSyncPlan(
      {
        commit: async (writes) => {
          operations.push(`set:${writes.map((write) => write.path).join(",")}`);
        },
        remove: async (paths) => {
          operations.push(`delete:${paths.join(",")}`);
        },
      },
      value,
    );
      ...
      executeSyncPlan(
        {
          commit: async (writes) => {
            committed.push(...writes.map((write) => write.path));
            if (writes.some((write) => write.path === "manifest/active"))
  ...
  it("leaves obsolete chunk ids in the active manifest when cleanup fails", async () => {
    ...
    await expect(
      executeSyncPlan(
        {
          commit: async (writes) => {
            commits.push(...writes.map((write) => write.data));
          },
          remove: async () => {
