...
export function readLastSyncedAtEpoch(
  manifest?: { syncedAtEpoch?: number; sourceSyncId?: string } | null,
): number | undefined {
  if (manifest?.syncedAtEpoch !== undefined && Number.isFinite(manifest.syncedAtEpoch))
    return manifest.syncedAtEpoch;
...
export function getSyncFreshness(
  lastSyncedAtEpoch: number | undefined,
  nowEpoch = Math.floor(Date.now() / 1000),
): { days: number; level: SyncFreshnessLevel } | null {
