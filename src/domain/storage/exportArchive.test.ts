...
import {
  ...
  type ArchiveReadPort,
  exportFirestoreArchive,
} from "./exportArchive";
...
describe("exportFirestoreArchive", () => {
  it("exports every known data layer and reads parts only for split pages", async () => {
    ...
    const collections: Record<string, ArchiveDocument[]> = {
      ...
      "players/100/snapshots/S1/pages/P1/parts": [
        { path: "players/100/snapshots/S1/pages/P1/parts/0001", data: { data: "{}" } },
      ],
    ...
    const roots: Record<string, ArchiveDocument> = {
      ...
      "players/100/manifests/matches": {
        path: "players/100/manifests/matches",
        data: { totalMatches: 1 },
      },
    };
    ...
    expect(archive.documents.map((document) => document.path)).toContain(
      "players/100/snapshots/S1/pages/P1/parts/0001",
    );
