...
describe("validateFirestoreArchive", () => {
  it("accepts an inline page even when object keys were reordered by Firestore", () => {
    const [page] = buildRawPageWrites({
      path: "players/100/snapshots/S1/pages/P1",
      metadata: {},
      raw: { z: 1, a: { y: 2, x: 3 } },
    });
  ...
  it("accepts complete split raw parts", () => {
    const pages = buildRawPageWrites({
      path: "players/100/snapshots/S1/pages/P1",
      metadata: {},
      raw: { payload: "🔥".repeat(200_000) },
    });
  ...
  it("rejects documents belonging to another player", () => {
    expect(() =>
      validateFirestoreArchive(archive([{ path: "players/200/matches/M1", data: {} }])),
    ).toThrow(/outside/);
  });
});
